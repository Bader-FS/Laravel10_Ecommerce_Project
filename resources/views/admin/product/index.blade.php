        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            @php $i = 1; @endphp
                ...
                <tr>
                  ...
                  <td><img src="{{\Illuminate\Support\Facades\Storage::url($product->image)}}" class="rounded mx-auto d-block" style="max-width:100px;" alt=""></td>
                  <td>
                    @if($product->status == 1)
                    <span class="badge badge-success">
                      {{trans('product_trans.showing')}}
                    </span>
                    @else
                    <span class="badge badge-danger">
                     {{trans('product_trans.hidden')}}
                    </span>
                    @endif
                  </td>
                  <td>
                    @if($product->trend == 1)
                    <span class="badge badge-success">
                     {{trans('product_trans.trending')}}
                    </span>
                    @else
                    <span class="badge badge-danger">
                     {{trans('product_trans.not_trending')}}
                    </span>
                    @endif
                  </td> 
                  <td>
